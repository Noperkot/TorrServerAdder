...
class tWorkerSrv {
	...
	constructor(msgPort) {
		...
		this.msgPort.onDisconnect.addListener(this.Abort);						// дисконнект происходит при вызове disconnect() в контент-скрипте, при уходе/обновлении/закрытии вкладки или по клику по окну предзагрузки
	...
	onMessage = (request) => {		
		...
		case 'torrAdd':
			...
			.catch((e) => {														// если в цепочке случилось исключение - шлем сообщение
				this.preventDrop();												// предотвращаем дроп после успешного добавлении
				this.pstMsg('message', {
					'title': chrome.i18n.getMessage(e.message) || e.message,	//  в лисе если не найдено в консоль выводится ошибка, и возвращается ''
